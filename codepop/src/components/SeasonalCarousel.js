...
const SeasonalCarousel = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
             try {
                ...
                // Safety check: ensure drinks is an array before mapping
    ...
    const createDrink = async (item) => {
        try {
            // FIX: Added 'const' to cartList
            const rawCart = await AsyncStorage.getItem("checkoutList");
            const currentList = rawCart ? JSON.parse(rawCart) : [];
            ...
            const response = await fetch(`${BASE_URL}/backend/drinks/`, {
              ...
              const drinkID = responseData.DrinkID;
            ...
            console.log("Create Drink Error:", error);
        }
    }
    ...
    const renderItem = ({ item }) => (
        <TouchableOpacity style={styles.carouselItem} onPress={() => createDrink(item)}>
            ...
            />
            <Text style={styles.drinkName}>{item.name}</Text>
            {/* Added safety check for price */}
            <Text style={styles.drinkPrice}>${(item.price || 0).toFixed(2)}</Text>
        </TouchableOpacity>
    );
    ...
    return (
        <View style={{ height: 250 }}>
            {data.length > 0 ? (
                ...
                <Text style={{ textAlign: 'center', marginTop: 20 }}>Loading seasonal drinks...</Text>
            )}
        </View>
    );
};
